...
import type { NewPosition, UpdatePosition } from "../types/Position";
...
export async function newPositionUpdate(
  id: number,
  newUpdatePosition: UpdatePosition
) {
  try {
    const response = await apiFetch(`${BASE_URL}/positions/${id}`, {
      method: "PUT",
      ...
      credentials: "include",
      body: JSON.stringify(newUpdatePosition),
    });
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
